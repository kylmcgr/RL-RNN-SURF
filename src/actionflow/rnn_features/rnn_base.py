...
class RNNBase():
    def __init__(self, a_size, s_size, n_cells):
        ...
        if s_size != 0:
            ...
            self.prev_stim = tf.placeholder(shape=[None, None, s_size], dtype=Const.FLOAT)

            self.prev_samp = tf.placeholder(shape=[None, None, s_size], dtype=Const.FLOAT)
            # self.prev_states_onehot = tf.one_hot(self.prev_states, s_size, dtype=Const.FLOAT)
            rnn_in = tf.concat(values=[self.prev_rewards, self.prev_actions_onehot, self.prev_states, self.prev_stim, self.prev_samp], axis=2)
