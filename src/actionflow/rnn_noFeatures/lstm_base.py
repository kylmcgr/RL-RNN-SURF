...
from actionflow.rnn_noFeatures.rnn_base import RNNBase
from ..rnn_noFeatures.consts import Const
...
class LSTMBase(RNNBase):
    ...
    def beh_feed(self, actions, states):
        ...
        feed_dict = {self.prev_actions: prev_actions,
                     self.actions: actions,
                     self.state_in[0]: np.repeat(rnn_state[0], prev_actions.shape[0], axis=0),
                     self.state_in[1]: np.repeat(rnn_state[1], prev_actions.shape[0], axis=0)}
