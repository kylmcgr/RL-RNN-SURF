...
from ..rnn_noReward.consts import Const
...
class RNNBase():
    def __init__(self, a_size, s_size, n_cells):
        ...
        self.n_batches = tf.shape(self.prev_actions)[0]
        ...
        if s_size != 0:
            ...
            rnn_in = tf.concat(values=[self.prev_actions_onehot, self.prev_states], axis=2)
            # DIM: nBatches x (nChoices + 1 ) x ( 1 + nActionTypes + nStateTypes)

        else:
            rnn_in = tf.concat(values=[self.prev_actions_onehot], axis=2)
    ...
    def simulate(self, sess, actions, states):
        """
        N:  number of batches
        ...
        feed_dict = self.beh_feed(actions, states)
    ...
    @abstractmethod
    def beh_feed(self, actions, states):
